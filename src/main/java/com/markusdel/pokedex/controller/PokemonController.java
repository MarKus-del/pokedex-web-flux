package com.markusdel.pokedex.controller;

import com.markusdel.pokedex.model.Pokemon;
import com.markusdel.pokedex.model.PokemonEvent;
...
public class PokemonController {
    ...
    @DeleteMapping("{id}")
    public Mono<ResponseEntity<Void>> deletePokemonById(@PathVariable(value= "id") String id){
        return repository.findById(id)
                .flatMap(existingPokemon ->
    ...
    public Flux<PokemonEvent> getPokemonEvents(){
        return Flux.interval(Duration.ofSeconds(5))
                .map(val ->
                        new PokemonEvent(val, "POkemonzitoos")
                );
    }
